...
architecture IMP of user_logic is
  ...
  signal enable_trubadur : std_logic;
begin
  ...
  begin
    if (Bus2IP_Resetn='0') then
        ...
        if (reg_we = '1') then
          case (unit_addr) is
            ...
            when REG_ADDR_06 => frame_color      <= Bus2IP_Data(23 downto 0);
				...
				when REG_ADDR_08 => enable_trubadur <= Bus2IP_Data(0);
            when others => null;
          end case;
        end if;
    end if;
	 ...
	 IRQ <= '1' when ((dir_pixel_row = v_sync_counter_tc(10 downto 0)) and (enable_trubadur = '1'))
			else '0';
--  direct_mode      <= '0';
--  display_mode     <= "01";
--  font_size        <= x"1";
